...
#define triggerTravelMillis 600             // how long should trigger be squeezed in ms (in semi-auto firing)
// higher value = longer trigger pull                                                                          

// more trigger settings:
#define fireDelayMillis 3000             // how long after firing before resetting in milliseconds. 
// Bigger number, longer delay between any ahots.                                                                          
...
int clipSize = 15;                          // how many shots before the gun will be empty and the gun will be disabled (reload switch resets the ammo counter)
...
int firing = 0;                       // are we cuurrently firing
unsigned long int fireTimer = 0;
unsigned long int fireStart = 0;      // the time the firing started
int fireSelector = 1;                 //   1 - semi-automatic firing, auto/semi-auto gun
                                      //   3 - full automatic firing, full-auto gun
unsigned long int lastFired = 0;      // Last time (in millis) the gun was fired
...
void loop() {   
  ...
  if ( fireSelector == 1 ) { 
    if ( fire == 1 && !clipEmpty && firing == 0 ) {           // if firing...
      fireStart = millis();
      if (( fireStart - lastFired ) > fireDelayMillis ) { // don't accept fire again until timer is up or its the first shot
        firing = 1;
      } 
    }
  }
  if ( fireSelector == 3 ) {
    if ( fire == 1 && !clipEmpty  ) {      
      firing = 1;
    } else { firing = 0; }  // have to stop automatic fire
  }
...
void Fire(int selector) {         // function to fire the gun, based on what firing mode is selected
  if(selector == 1) {
    ...
    if(fireTimer >= 1.5*triggerTravelMillis) {
      digitalWrite(firingIndicatorLEDPin, LOW); // have the LED stay on a bit longer
      firing = 0;
      lastFired = millis();
      if(useAmmoCounter) {
...
void assignPins() {
  if(type == "Arduino_bare" || type == "Arduino_Bare") {
    // pin attachments:
    panServoPin = 7;                        // Arduino pin for pan servo
    tiltServoPin = 8;                       // Arduino pin for tilt servo
    triggerServoPin = 11;                    // Arduino pin for trigger servo, or output to trigger MOSFET
    firingIndicatorLEDPin = 2;              // Arduino pin for firing indicator LED
    USBIndicatorLEDPin = 3;                 // Arduino pin for USB indicator LED
    modeIndicatorLEDPin = 4;                // Arduino pin for Mode indicator LED
    reloadSwitchPin = 12;                    // Arduino pin for input from RELOAD switch
    disablePlatePin = 9;                    // Arduino pin for input from disable plate
